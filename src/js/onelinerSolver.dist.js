...
(() => {
/*!**********************************!*\
  !*** ./src/js/onelinerSolver.js ***!
  \**********************************/
var _ = __webpack_require__(/*! lodash */ "./node_modules/lodash/lodash.js");

var GeneticAlgorithmConstructor = __webpack_require__(/*! geneticalgorithm */ "./node_modules/geneticalgorithm/index.js");
...
function mutation(individual) {
  var position1 = _.random(0, individual.genes.length - 1);

  var position2 = _.random(0, individual.genes.length - 1);

  var temp = individual.genes[position1];
...
function fitness(individual) {
  var fitness = 1; // current node will be the common node between the first two edges

  var currentNode = _.intersection(edges[individual.genes[0]], edges[individual.genes[1]])[0];

  if (!currentNode) {
    return fitness;
  }

  for (var i = 1; i < individual.genes.length; i++) {
    var edge = individual.genes[i]; // if the current node is not in the edge, then the edge is not connected to the current node
...
function crossover(parentA, parentB) {
  var crossoverPoint = _.random(1, parentA.genes.length - 1);

  var parentAFirstHalf = _.take(parentA.genes, crossoverPoint);

  var parentASecondHalf = _.takeRight(parentA.genes, parentA.genes.length - crossoverPoint);

  var parentBFirstHalf = _.take(parentB.genes, crossoverPoint);

  var parentBSecondHalf = _.takeRight(parentB.genes, parentB.genes.length - crossoverPoint);
